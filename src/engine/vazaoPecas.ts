 ...
 *
 * A física de "quanto passa" em cada elemento fora da rede de junções: tubo por gravidade (Torricelli/atrito, com refluxo e ladrão), bomba (curva/ponto de
 * operação e múltiplas saídas), fonte (vazão fixa com boia) e consumo (demanda limitada pela capacidade do cano). O `simulador.ts` (o tick) orquestra estas
 * funções; a rede que bifurca/une em junções vive em `redeJuncoes.ts`.
 *
 * Cada função é PURA quanto ao estado do tick: recebe o índice de grafo, a peça e os acumuladores (fluxos/vazões/alertas) por parâmetro — nada de fechar sobre o
...
export function calcularTubo(
  ...
  // Sem reservatório a montante não há coluna d'água que gere fluxo por gravidade. Se o lado de montante é uma fonte/bomba (não-reservatório), quem
  ...
  // Tubo com conexão a jusante que NÃO alcança um reservatório (leva a uma bomba ou consumo) não é descarga livre ao ambiente — o elemento ativo governa esse
  ...
  // Altura em que o tubo toca cada reservatório (relativa à base). Uma tomada em altura só escoa a água ACIMA dela.
  ...
  // Fluxo natural origem→destino. A origem precisa ter água ACIMA do seu bocal (senão o bocal "chupa ar" e nada sai). A água descarrega no MAIOR entre a
  // superfície e o bocal do destino: um bocal alto exige mais carga para ser vencido (não dá para empurrar água acima da própria superfície da origem).
...
/**
 * Coleta os IDs dos tubos ABERTOS (não-ladrão) de uma mesma cadeia em série a partir de `tuboInicial`: tubos ligados diretamente ou por junções, sem um
 * reservatório/elemento ativo no meio. Um tubo de registro fechado ou ladrão é fronteira (quebra a cadeia) e não é incluído.
 */
export function coletarCadeiaTubos(idx: GrafoIndex, tuboInicial: string): string[] {
  ...
  while (fila.length > 0) {
    ...
    for (const v of vizinhos) {
      ...
      if (!vp) continue;
      // Atravessa junções (sem volume) e tubos abertos não-ladrão; para em reservatório/bomba/fonte/consumo e em tubo de registro fechado/ladrão.
      const atravessa =
        vp.tipo === 'juncao' ||
        (isTubo(vp) &&
          !vp.props.ladrao &&
...
export function calcularBomba(
  ...
  // Fonte de sucção respeitando válvulas em série (registro/boia). Sem origem alcançável ou com o caminho fechado, a bomba não move nada.
  const upPath = idx.resolverFluxo(bomba.id, 'up');
  if (!upPath.res || !upPath.aberto) return 0;
  const up = upPath.res;
  // Origem vazia com a bomba ligada = RODANDO A SECO. Não move nada (evita vazão fantasma) e sinaliza o alerta/log — a proteção fica a cargo da boia reversa.
  ...
  // Uma bomba pode alimentar múltiplas saídas: reservatórios (recalque) ou pontos de CONSUMO (ex.: bomba de incêndio → hidrantes). A vazão nominal é
  // dividida só entre as saídas ABERTAS. Uma saída para consumo só conta se o consumo estiver aberto e com demanda > 0 — por isso "consumo 0 = a bomba não
  ...
  for (const { c, dp } of abertas) {
    ...
    if (dp.res) {
      const liftM = cargaM(dp.res, kL) - hUp; // carga estática (m) a vencer nesta saída Curva: `alturaNominal` (plaquinha) deriva o k automaticamente e tem
      ...
      // Saída para consumo: a bomba entrega a MENOR entre a sua vazão (parcela) e a demanda. Se a demanda excede a vazão da bomba, ela não acompanha →
...
export function calcularConsumo(
  ...
  // calcularTubo o trataria como ralo para o ambiente, drenando o reservatório na vazão cheia da gravidade e ignorando a demanda do consumo. É QUEM o
  ...
  // Realismo: a saída é limitada pela CAPACIDADE do cano mais estreito no caminho (Torricelli pelo diâmetro e pela carga). Canos finos estrangulam.
