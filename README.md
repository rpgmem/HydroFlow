...
- **[Golpe de aríete](https://pt.wikipedia.org/wiki/Golpe_de_ar%C3%ADete) (alerta)** — sobrepressão de **[Joukowsky](https://pt.wikipedia.org/wiki/Nikolai_Jukovski)** numa parada súbita: `ΔP = ρ·a·v`. A celeridade `a` depende do **material** do tubo: `~500 m/s` no **PVC/plástico** (elástico, amortece) e `~1000–1300 m/s` em metal/concreto (rígidos). O tubo fica **vermelho** quando essa sobrepressão passaria do teto de pressão (⚙ Opções, padrão ≈ PN10 = 1000 kPa, ou a `pressaoNominal` do tubo); o log registra o **risco** e o **instante da parada brusca**. Alerta — o motor é quase-estático, não simula a onda.
