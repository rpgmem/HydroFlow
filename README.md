...
| `bomba` | `vazaoNominal`, `alturaNominal?` (altura de recalque; deriva a curva — a altura reduz a vazão), `curva?: {k}` (curva explícita; legado), `modeloBomba?` (preset do catálogo `bombasCatalogo.ts` — preenche vazão/altura/NPSH), `npshRequerido?` (m; liga o alerta de cavitação — usa a `cota` da bomba na carga de sucção), `sensores: string[]`, `modoControle?` (`auto`\|`ligado`\|`desligado`), `ligada?`, `revezamento?` (dupla alternada: metades "1"/"2" que se revezam a cada acionamento) |
...
| `alivio` | `pressaoAbertura` (kPa; setpoint de abertura), `diametro?` (mm; orifício de descarga, default 25) — válvula de alívio: descarrega ao ambiente quando a pressão local (coluna acima da `cota` da válvula) passa do setpoint; autolimitante |
...
- **[Golpe de aríete](https://pt.wikipedia.org/wiki/Golpe_de_ar%C3%ADete) (alerta)** — sobrepressão de **[Joukowsky](https://pt.wikipedia.org/wiki/Nikolai_Jukovski)** numa parada súbita: `ΔP = ρ·a·v`. A celeridade `a` depende do **material** do tubo: `~500 m/s` no **PVC/plástico** (elástico, amortece) e `~1000–1300 m/s` em metal/concreto (rígidos). O Joukowsky cheio só vale para fechamento **instantâneo** — na prática só o **desarme de bomba** é abrupto; registros (manuais) e boias (flutuador) fecham devagar e a gravidade cessa gradual, então esses tubos têm o surto **atenuado** (fator `ATENUACAO_GOLPE_LENTO`). O tubo fica **vermelho** quando a sobrepressão efetiva passaria do teto de pressão (⚙ Opções, padrão ≈ PN10 = 1000 kPa, ou a `pressaoNominal` do tubo); o log registra o **risco** e o **instante da parada brusca**. Alerta — o motor é quase-estático, não simula a onda.
- **[NPSH](https://pt.wikipedia.org/wiki/NPSH) / cavitação (alerta)** — o NPSH disponível na sucção `NPSH_disp = (P_atm − P_vapor)/(ρ·g) + carga_sucção` (carga de sucção = `cota + nivel` da fonte − `cota` da bomba − perdas por atrito na sucção) precisa superar o `npshRequerido` da bomba (dado de catálogo). Abaixo disso a pressão na entrada cai à **[pressão de vapor](https://pt.wikipedia.org/wiki/Press%C3%A3o_de_vapor)** da água (por **Tetens**, cresce com a temperatura) e ela "ferve": a bomba fica **âmbar** e o log registra o risco. Informe o `npshRequerido` no inspetor da bomba para ligar o alerta (em branco = sem checagem).
- **[Válvula de alívio](https://pt.wikipedia.org/wiki/V%C3%A1lvula_de_al%C3%ADvio)** — peça de proteção ligada a jusante de um reservatório: abre e descarrega ao ambiente quando a pressão local (coluna acima da `cota` da válvula) passa do setpoint `pressaoAbertura`. Descarga por **Torricelli pelo orifício** sobre o excedente de carga acima do setpoint (autolimitante: drena a origem até a pressão voltar ao setpoint). Fica **vermelha** e registra no log ao abrir.
...
- **[Pressão na base](https://pt.wikipedia.org/wiki/Press%C3%A3o_hidrost%C3%A1tica) ([Teorema de Stevin](https://pt.wikipedia.org/wiki/Simon_Stevin))** — `P = ρ·g·h` (readout no inspetor do reservatório, na unidade de pressão escolhida em ⚙ Opções: kPa, m.c.a. ou psi). Só exibição — não entra no cálculo de vazão.
...
A física roda em **[SI](https://pt.wikipedia.org/wiki/Sistema_Internacional_de_Unidades)** (metros, m³, m³/s) e o **armazenamento também é canônico em SI**: todas as magnitudes do projeto ficam em SI e o campo `unidades` é apenas a **preferência de exibição** — a UI converte SI↔exibição. Assim, **trocar a unidade preserva o valor físico** (não reinterpreta o número). Opções de exibição: comprimento **m/cm**, volume/vazão **L ou m³**, pressão **kPa / m.c.a. / psi**; o **diâmetro de tubo é sempre em milímetros**. Projetos salvos em versões anteriores são migrados automaticamente ao carregar (schema ≥ 1.2.0). O multiplicador de velocidade (1x…120x) permite acompanhar cenários realistas (vazões em L/s enchendo tanques de milhares de litros) em segundos.
